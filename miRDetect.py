...
from config import path_db, db_name, path_blast
...
path_mirbase = os.path.join(path_db, "mature")
run_mirna_blast = subprocess.run(["./blastn", "-query", Assembly_file, "-db", path_mirbase, "-evalue", "0.001", "-word_size", "6", "-outfmt", "6", "-max_target_seqs", "1", "-out", blast_file])
...
path_uniprot = os.path.join(path_db, "uniprot")
run_blastx = subprocess.run(["./blastx", "-query", final_file, "-db", path_uniprot, "-evalue", "0.001", "-outfmt", "6", "-max_target_seqs", "1", "-out", output])
...
if os.path.getsize('output.tsv') > 0:
    ...
    print("*********Predicting precursor sequences*************")
    predictseq("features.csv")
    gdx_path = os.path.join(path_tool, "candidate-mirna.fasta.gdx")
    flat_path = os.path.join(path_tool, "candidate-mirna.fasta.flat")
    feature = os.path.join(path_tool, "features.csv")
    os.remove(output)
    os.remove(Temp)
    os.remove(result_file)
